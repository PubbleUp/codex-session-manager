...
func (a App) HideProject(projectPath string) error {
	if err := config.HideProject(a.Config, projectPath); err != nil {
		_ = a.Audit("hide-project", "", projectPath, "", "failed", err.Error())
		return err
	}
	_ = a.Audit("hide-project", "", projectPath, "", "success", "")
	return nil
}

func (a App) UnhideProject(projectPath string) error {
	if err := config.UnhideProject(a.Config, projectPath); err != nil {
		_ = a.Audit("unhide-project", "", projectPath, "", "failed", err.Error())
		return err
	}
	_ = a.Audit("unhide-project", "", projectPath, "", "success", "")
	return nil
}
...
func (a App) Backup(sessionID string) (domain.BackupManifest, error) {
	...
	if !ok {
		return domain.BackupManifest{}, fmt.Errorf("session not found: %s", sessionID)
	}
	manifest, err := backupsvc.BackupSession(a.Config, session)
	if err != nil {
		_ = a.Audit("backup", session.ID, session.FilePath, "", "failed", err.Error())
		return domain.BackupManifest{}, err
	}
	_ = a.Audit("backup", session.ID, session.FilePath, filepath.Join(a.Config.BackupDir, session.ID), "success", "")
	return manifest, nil
}
...
func (a App) Restore(sessionID string) (restoresvc.Result, error) {
	...
	if err != nil {
		_ = a.Audit("restore", sessionID, "", "", "failed", err.Error())
		return result, err
	}
	_ = a.Audit("restore", sessionID, result.SourcePath, result.TargetPath, "success", result.Message)
	return result, nil
}
...
func (a App) RestoreSession(session domain.SessionRecord) (restoresvc.Result, error) {
	...
	if err != nil {
		_ = a.Audit("restore", session.ID, session.FilePath, "", "failed", err.Error())
		return result, err
	}
	_ = a.Audit("restore", session.ID, result.SourcePath, result.TargetPath, "success", result.Message)
	return result, nil
}
...
func (a App) Remove(sessionID string) (removesvc.Result, error) {
	...
	if !ok {
		return removesvc.Result{}, fmt.Errorf("visible session not found: %s", sessionID)
	}
	result, err := removesvc.RemoveFromCurrentCodex(a.Config, session)
	if err != nil {
		_ = a.Audit("remove", session.ID, session.FilePath, "", "failed", err.Error())
		return result, err
	}
	_ = a.Audit("remove", session.ID, result.Source, result.Target, "success", result.Message)
	return result, nil
}
...
func (a App) RemoveSession(session domain.SessionRecord) (removesvc.Result, error) {
	if session.Source == domain.SessionSourceBackup || session.Source == domain.SessionSourceRemoved {
		...
		if err != nil {
			_ = a.Audit("purge", session.ID, session.FilePath, "", "failed", err.Error())
			return result, err
		}
		_ = a.Audit("purge", session.ID, result.Source, "", "success", result.Message)
		return result, nil
	}
	if session.Source == domain.SessionSourceArchived {
		result, err := removesvc.PurgeArchivedSession(a.Config, session)
		if err != nil {
			_ = a.Audit("purge-archived", session.ID, session.FilePath, "", "failed", err.Error())
			return result, err
		}
		_ = a.Audit("purge-archived", session.ID, result.Source, "", "success", result.Message)
		return result, nil
	}
	return a.Remove(session.ID)
}
...
func (a App) ArchiveSession(session domain.SessionRecord) (codex.CommandResult, error) {
	...
	if err != nil {
		_ = a.Audit("archive", session.ID, session.FilePath, "", "failed", err.Error())
		return result, err
	}
	_ = a.Audit("archive", session.ID, session.FilePath, "", "success", result.Message)
	return result, nil
}
...
func (a App) RenameSession(session domain.SessionRecord, name string) error {
	if err := codex.RenameSession(a.Config, session, name); err != nil {
		_ = a.Audit("rename", session.ID, session.FilePath, "", "failed", err.Error())
		return err
	}
	_ = a.Audit("rename", session.ID, session.FilePath, "", "success", name)
	return nil
}
...
func (a App) Repair(projectPath string) (restoresvc.RepairReport, error) {
	...
	for _, result := range report.Restored {
		_ = a.Audit("repair", result.SessionID, result.SourcePath, result.TargetPath, "success", result.Message)
	}
	for _, failed := range report.Failed {
		_ = a.Audit("repair", "", "", "", "failed", failed)
	}
	return report, nil
}
